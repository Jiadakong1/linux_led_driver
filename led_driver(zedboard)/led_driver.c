...
#define MY_GPIO_PHY_ADDR    0x41200000 //Modify the address to your peripheral
#define MY_GPIO_REG_NUM     2
...
{
    int i;
    iowrite32((unsigned int)0xff, GPIO_Regs);
...
int __init dev_led_init(void)
{
    ...
    GPIO_Regs = ioremap(MY_GPIO_PHY_ADDR, MY_GPIO_REG_NUM); /* Verify it's non-null! */
    if(GPIO_Regs == NULL)
    {
    ...
    iowrite32((unsigned int)0x55, GPIO_Regs);
